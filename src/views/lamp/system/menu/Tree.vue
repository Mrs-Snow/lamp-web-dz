<template>
  <div class="bg-white m-4 mr-2 overflow-hidden">
    ...
    <BasicTree
      :title="t('lamp.system.menu.table.title')"
      toolbar
      checkable
      search
      checkStrictly
...
<script lang="ts">
  ...
  import { tree, remove } from '/@/api/lamp/system/menu';
        ...
        return [
          {
            ...
            handler: () => {
              emit('add', findNodeByKey(unref(node.$attrs).id, treeData.value), treeData.value);
            },
            icon: 'bi:plus',
          },
          {
